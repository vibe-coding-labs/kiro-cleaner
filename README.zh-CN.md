...
## 🚀 Kiro IDE 变卡了？一键清理，立即提速！

**当对话历史文件积累过多时，Kiro IDE 会明显变慢、卡顿。** Kiro Cleaner 是专门为解决这个问题而设计的工具，通过安全清理冗余的对话数据、缓存文件和日志，让你的 Kiro IDE 重新起飞！

### 💡 为什么需要 Kiro Cleaner

- **🐌 问题**：随着使用 Kiro IDE，对话历史会在数据库中不断积累，导致 IDE 变慢、卡顿，甚至卡死
- **⚡ 解决方案**：Kiro Cleaner 智能清理冗余数据，**立即恢复 IDE 响应速度，性能显著提升**
- **🛡️ 安全保障**：清理前自动备份，随时可以恢复 - 再也不用担心数据丢失
...
- 🧹 **安全清理**: 基于规则的智能清理，确保数据安全，**解决IDE卡顿问题**
...
### 1. 🐌 IDE 运行缓慢
**症状**：Kiro IDE 响应变慢、卡顿严重，甚至卡死  
**原因**：本地缓存和对话历史过大是主要原因！  
**解决方案**：使用 Kiro Cleaner 清理冗余数据后，**IDE 立即起飞，响应速度显著提升**

### 2. 💾 存储空间不足
**症状**：你的磁盘空间告急  
**解决方案**：Kiro Cleaner 可以清理对话历史、缓存和临时文件，**释放大量存储空间**

### 3. 🔄 项目切换频繁
**症状**：你在多个项目间切换  
**解决方案**：定期清理可以**避免项目数据混乱，保持 IDE 运行流畅**

### 4. 🛠️ 长期使用维护
**症状**：你长期使用 Kiro IDE  
**解决方案**：定期维护可以**防止数据累积，避免性能下降**
...
以下是在终端中运行 `kiro-cleaner scan` 命令的演示：
...
从 [Releases](https://github.com/vibe-coding-labs/kiro-cleaner/releases) 页面下载适合您操作系统的预编译二进制文件。
...
1. 查看 [故障排除](#故障排除) 部分
2. 搜索已有的 [Issues](https://github.com/vibe-coding-labs/kiro-cleaner/issues)
3. 创建新的 [Issue](https://github.com/vibe-coding-labs/kiro-cleaner/issues/new)
4. 参与 [Discussions](https://github.com/vibe-coding-labs/kiro-cleaner/discussions)
